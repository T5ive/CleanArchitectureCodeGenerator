...
public sealed class CodeGeneratorPackage : AsyncPackage
{
	...
	protected async override System.Threading.Tasks.Task InitializeAsync(CancellationToken cancellationToken, IProgress<ServiceProgressData> progress)
	{
		...
		{
			CommandID menuCommandID = new(PackageGuids.guidCodeGeneratorCmdSet, PackageIds.cmdidMyCommand);
			OleMenuCommand menuItem = new(ExecuteAsync, menuCommandID);
	...
	private void ExecuteAsync(object sender, EventArgs e)
	{
		...
		NewItemTarget ui = NewItemTarget.Create(_dte, "WebUI");
		var includes = new[] { "BaseEntity", "BaseAuditableEntity", "BaseSoftDeleteEntity" };
		var objectList = domain.Project.GetEntities()
	...
	private void AddProjectFolder(string name, NewItemTarget target)
	{
		...
		foreach (string segment in SplitPath(name))
		{
			...
			ProjectItem folder = items
								 ...
								 .FirstOrDefault(item => item.IsKind(Constants.vsProjectItemKindPhysicalFolder, Constants.vsProjectItemKindVirtualFolder)) ??
								 items.AddFromDirectory(parentDirectory);
